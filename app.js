...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function updateIntervals() {
        ...
        intervalsDiv.innerHTML = intervals.map((interval, index) => `
            <div class="interval">
                ...
                <input type="number" placeholder="Cadence (RPM)" value="${interval.cadence || ''}" data-index="${index}" data-type="cadence">
                <input type="text" placeholder="Duration (MM:SS)" value="${interval.duration || ''}" data-index="${index}" data-type="duration">
                <button type="button" class="removeInterval" data-index="${index}">Remove</button>
            </div>
        `).join('');
    }
    ...
    function saveTraining() {
        ...
        if (!trainingName || intervals.length === 0) {
            alert('Please enter a name and add at least one interval.');
            return;
        }
    ...
    function populateTrainingList() {
        trainingList.innerHTML = trainings.map((training, index) => `
            <li>
                ${training.name}
                <button class="editTraining" data-index="${index}">Edit</button>
            </li>
        `).join('');
    }
    ...
    async function connectDevice() {
        try {
            ...
            deviceStatus.innerText = 'Connected to ' + device.name;
        } catch (error) {
            deviceStatus.innerText = 'Connection failed';
            console.error(error);
        }
    }
    ...
    function runNextInterval() {
        ...
        const durationParts = interval.duration.split(':');
        const durationValue = (parseInt(durationParts[0]) * 60 + parseInt(durationParts[1])) * 1000;

        document.getElementById('currentWattage').innerText = wattage;
        document.getElementById('currentCadence').innerText = cadenceValue;
        document.getElementById('currentIntervalTime').innerText = interval.duration;
        document.getElementById('totalTimeRemaining').innerText = training.intervals.length * durationValue / 1000 + ' sec';

        start_erg_mode(wattage); // Start the ERG mode with the calculated wattage
    ...
    document.getElementById('trainingList').addEventListener('click', (e) => {
        if (e.target.classList.contains('editTraining')) {
            const index = e.target.dataset.index;
            editTraining(index);
        }
    });
